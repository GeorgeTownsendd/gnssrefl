...
This page describes the `extract_arcs` module and highlights some useful features. For the most up to date documentation please visit the [autogenerated API reference](https://gnssrefl.readthedocs.io/en/latest/api/gnssrefl.extract_arcs.html).
...
# 237 arcs extracted, 50 passed QC
...
pick = lambda arcs: min(arcs, key=lambda x: abs(x[0]['az_init'] - 187))

fig, ax = plt.subplots(figsize=(7, 3))
for arcs, doy in [(p038_doy183, 183), (p038_doy184, 184)]:
    meta, data = pick(arcs)
    p = meta['phase_processing_results']
    ax.plot(np.sin(np.radians(data['ele'])), data['snr'], lw=0.8, label=f"DOY {doy}  phase={p['Phase']:.0f}" + r"$^\circ$")
ax.set_xlabel('sin(elevation)')
ax.set_ylabel('Detrended SNR')
